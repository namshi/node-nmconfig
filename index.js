...
var defaultFiles = appPkgConf.nmConfig.defaultFiles || ['base', 'dev.example', 'dev', 'staging', 'live'];
var shellVars = {
  env: appName + '_ENV',
  reconfigPrefix: appName + '_CONFIG'
};
var env = process.env[shellVars.env]|| process.env.NODE_ENV || 'dev';

if (!_isArray(defaultFiles)) {
...
function ensureFile(fileName) {
  ...
  (fs.existsSync(from)) && options.from = from;
...
function combineFiles(files) {
  ...
  files.forEach(function(file) {
    var configFilePath = path.join(configDir, configFile + '.yml');
